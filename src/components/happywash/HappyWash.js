...
function QrGenerate() {
    ...
    const designQRElement = async (site) => {
        ...
        if (canvas) {
            ...
            downloadlink.download = site + `_happywash.png`;
    ...
    const generateNewQr = async (e) => {
        setAlert(0)
        if (sitenameR.current.value !== '' && sitecodeR.current.value !== '') {
            var valuelink = "https://www.otr.com.au/otr-app/?path=goto&page=happywash&site=" + sitecodeR.current.value;
            setSiteCode(1)
            setQRDesc(valuelink);
