...
import './Happywash_MultiMashine.css'
...
function QrGenerate() {
    ...
    const designQRElement = async (site, allmachines) => {
        ...
        context.fillText(site, 45, 875);
            context.fillText("Machine #" + machineNo, 670, 875);
            if (canvas) {
                const pngUrl = canvas
                    .toDataURL("image/png")
                ...
                downloadlink.download = site + '_pump-' + machineNo + `.png`;
    ...
    const generateNewQr = async (e) => {
        ...
        if (sitenameR.current.value !== '' && sitecodeR.current.value !== '' && numberofQrR.current.value !== '' && numberofQrR.current.value !== 0) {
            ...
            for (let i = 1; i <= numberofQrR.current.value; i++) {
                var newObj = {
                    id: 'QR' + i,
                    valuelink: "https://www.otr.com.au/otr-app/?path=goto&page=happywash&site=" + sitecodeR.current.value + "&m=" + i
                }
            ...
            setTimeout(() => {
                designQRElement(site, machineArray)
            }, 1000);
            setHideForm(1)
        } else {
            setAlert(1)
            ...
            <div className="qrform">
                {
                    ...
                    (
                        <div>
                            ...
                            >
                                <TextField required className="textfield" fullWidth label={<span className={'textlabel'}>Number of machines</span>} id="numberof" inputRef={numberofQrR} />
                            </Box>
                        </div>
                    )
                }
